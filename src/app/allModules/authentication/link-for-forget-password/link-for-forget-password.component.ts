...
export class LinkForForgetPasswordComponent implements OnInit {
  ...
  SendResetLink(): void {
    if (this.forgotPasswordForm.valid) {
      ...
      this.emailModel.UserName = this.forgotPasswordForm.get('email').value;
      ...
      this._authService.SendResetLinkToMail(this.emailModel).subscribe(
        (data) => {
          this.notificationSnackBarComponent.openSnackBar(`Reset password link sent successfully to ${this.emailModel.UserName}`, SnackBarStatus.success);
          this.ResetControl();
          this.IsProgressBarVisibile = false;
