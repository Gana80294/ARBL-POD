...
export class ReportService {
  ...
  GetFilteredInvoiceDetail(UserID: Guid, CurrentPage: number, Records: number, Status: string, StartDate: string, EndDate: string, InvoiceNumber: string,
    Organization: string, Division: string, Plant: string, CustomerName: string,CustomerGroup:string): Observable<ReportInvoice[] | string> {
    return this._httpClient.get<ReportInvoice[]>(
      `${this.baseAddress}api/Report/GetFilteredInvoiceDetails?UserID=${UserID}&CurrentPage=${CurrentPage}&Records=${Records}&Status=${Status}&StartDate=${StartDate}&EndDate=${EndDate}&InvoiceNumber=${InvoiceNumber}&Organization=${Organization}&Division=${Division}&Plant=${Plant}&CustomerName=${CustomerName}&CustomerGroup=${CustomerGroup}`
    )
      .pipe(catchError(this.errorHandler));
  }
