...
import { ChangePassword, ForgotPassword, EMailModel, RolewithGroup } from 'app/models/master';
...
export class AuthService {
  ...
  sendSMSforOTP(data):Observable<any>{
    return this._httpClient.post<any>(this.baseAddress+"/api/Master/PasswordResetSendSMSOTP",data).pipe(
      catchError(catchError(this.errorHandler1))
    )
     }
     GetCustomerGroupsWithRoleNames():Observable<any>{
      return this._httpClient.get<RolewithGroup[]>(`${this.baseAddress}api/Master/GetRoleandGroups`)
        }
     changePasswordWithSMSOTP(data):Observable<any>{
    return this._httpClient.post<any>(this.baseAddress+"/api/Master/ResetPasswordWithSMSOTP",data).pipe(
      catchError(catchError(this.errorHandler1))
    )
     }
